...
def main():
    ...
    parser.add_argument('-negNum', action='store', dest='negNum', default=7, type=int)   #负样本的数目
...
class Model:
    ...
    def add_model(self):
        user_input = tf.nn.embedding_lookup(self.user_item_embedding, self.user)    #找到张量里索引对应位置,读取的是一个用户的数据
        item_input = tf.nn.embedding_lookup(self.item_user_embedding, self.item)

        def init_variable(shape, name):
            return tf.Variable(tf.truncated_normal(shape=shape, dtype=tf.float32, stddev=0.01), name=name)    #产生正态分布随机数
    ...
    def add_train_step(self):
        ...
        self.train_step = optimizer.minimize(self.loss)    #用Adam下降法优化loss，使网络输出训练步骤

    def init_sess(self):
        self.config = tf.ConfigProto()    #记录设备指派情况
        self.config.gpu_options.allow_growth = True
        self.config.allow_soft_placement = True      #自动选择运行设备
        self.sess = tf.Session(config=self.config)     #动态申请显存
        self.sess.run(tf.global_variables_initializer())    #配合tf.Variable的环境下使用，因为tf中建立的变量是没有初始化的，也就是在debug时还不是一个tensor量，而是一个Variable变量类型
                                                            #运行操作并评估提取的张量
        self.saver = tf.train.Saver()      #训练网络之后保存训练好的模型，以及在程序中读取已保存好的模型，存入checkpoint
        if os.path.exists(self.checkPoint):
            [os.remove(f) for f in os.listdir(self.checkPoint)]    #用于删除指定路径下的文件
        else:
    ...
    def run_epoch(self, sess, verbose=10):
        ...
        shuffled_idx = np.random.permutation(np.arange(train_len))  #对列表随机排序
        ...
        for i in range(num_batches):
            ...
            _, tmp_loss = sess.run([self.train_step, self.loss], feed_dict=feed_dict)    #猜测返回最好的那个loss和对应的train_step
            losses.append(tmp_loss)
            if verbose and i % verbose == 0:
                sys.stdout.write('\r{} / {} : loss = {}'.format(
                    i, num_batches, np.mean(losses[-verbose:])
                ))
                sys.stdout.flush()       #缓存区每隔一段时间输出一次
        loss = np.mean(losses)
    ...
    def evaluate(self, sess, topK):
        ...
        for i in range(len(testUser)):
            ...
            ranklist = heapq.nlargest(topK, item_score_dict, key=item_score_dict.get)   #返回前topK个最大的元素，按顺序排好
